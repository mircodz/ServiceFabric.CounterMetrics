...
internal static class CounterCatalog
{
    ...
    [
        ...
        [
            ...
            new("Checkpoint File Write Bytes/sec", "service_fabric.tstore.checkpoint.written",         Kind.Gauge,      "By/s"),
            new("Copy Disk Transfer Bytes/sec",    "service_fabric.tstore.copy.transferred",           Kind.Gauge,      "By/s"),
        ]),
...
internal static class InstanceParsers
{
    ...
    public static ParsedInstance? PartitionReplica(string instance)
    {
        ...
        if (p.Length < 2 || !Guid.TryParse(p[0], out var pid) || !long.TryParse(p[1], out var rid))
            return null;
        return new ParsedInstance(pid, rid, Tags(pid, rid, instance));
    }

    // {PartitionId}:{ReplicaId}:{StateProviderId}_{Differentiator}[_{StateProviderName}]
    public static ParsedInstance? TStore(string instance)
    {
        ...
        var provider = p[2].Split(['_'], 3);
        if (provider.Length < 2 || provider[0].Length == 0 || provider[1].Length == 0)
            return null;

        return new ParsedInstance(pid, rid, Tags(pid, rid, instance,
            new KeyValuePair<string, object?>("service_fabric.state_provider_id", provider[0])));
    }
    ...
    // {MethodName}_{MethodId}_{PartitionId}_{ReplicaOrInstanceId}_{InternalId}
    public static ParsedInstance? ServiceMethod(string instance)
        => ParseMethod(instance, includesReplica: true);
    ...
    public static ParsedInstance? PartitionInternal(string instance)
    {
        ...
        if (p.Length < 2 || !Guid.TryParse(p[0], out var pid))
            return null;
        return new ParsedInstance(pid, null, Tags(pid, null, instance));
    }

    // {MethodName}_{MethodId}_{PartitionId}_{InternalId}
    public static ParsedInstance? ActorMethod(string instance)
        => ParseMethod(instance, includesReplica: false);

    private static ParsedInstance? ParseMethod(string instance, bool includesReplica)
    {
        var p = instance.Split('_');
        var n = p.Length;
        var suffixLength = includesReplica ? 4 : 3;
        if (n <= suffixLength)
            return null;

        var methodIdIndex = n - suffixLength;
        if (!Guid.TryParse(p[methodIdIndex + 1], out var pid) ||
            p[methodIdIndex].Length == 0 || p[n - 1].Length == 0)
            return null;

        long? rid = null;
        if (includesReplica)
        {
            if (!long.TryParse(p[methodIdIndex + 2], out var replicaId))
                return null;
            rid = replicaId;
        }

        var method = string.Join("_", p, 0, methodIdIndex);
        if (method.Length == 0)
            return null;

        return new ParsedInstance(pid, rid, Tags(pid, rid, instance,
            new KeyValuePair<string, object?>("service_fabric.method", method),
            new KeyValuePair<string, object?>("service_fabric.method_id", p[methodIdIndex])));
    }

    private static KeyValuePair<string, object?>[] Tags(Guid pid, long? rid, string instance, params KeyValuePair<string, object?>[] extra)
    {
        var tags = new KeyValuePair<string, object?>[(rid.HasValue ? 3 : 2) + extra.Length];
        var index = 0;
        tags[index++] = new KeyValuePair<string, object?>(PartitionTag, pid.ToString());
        if (rid is long replicaId)
            tags[index++] = new KeyValuePair<string, object?>(ReplicaTag, replicaId);

        // Opaque suffixes distinguish counters with the same parsed partition/provider/method.
        tags[index++] = new KeyValuePair<string, object?>("service_fabric.counter_instance", instance);
        Array.Copy(extra, 0, tags, index, extra.Length);
        return tags;
    }
}
